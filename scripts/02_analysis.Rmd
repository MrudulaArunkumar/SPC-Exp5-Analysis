...
write_csv(file = here("data","AwareSummary.csv"),AwareSummary)
...
Phase31TCraw_Awareness <- Phase31_TC %>% 
  mutate(SSCRAwareness = ifelse(PROLIFIC_PID %in% SS_CR_aware, "Aware","Unaware"),
         S1TRAwareness = ifelse(PROLIFIC_PID %in% S1_Q_aware, "Transferred","Not transferred"),
         S1CRAwareness = ifelse(PROLIFIC_PID %in% S1C_Q_aware, "Aware","Unaware"),
         S2Awareness = ifelse(PROLIFIC_PID %in% S2_Q_aware, "Aware","Unaware"))
Phase31TCraw_Awareness$S1TRAwareness <- factor(Phase31TCraw_Awareness$S1TRAwareness, levels = c("Transferred", "Not transferred"))

Phase31TCraw_Awareness <- Phase31TCraw_Awareness %>% 
  select(PROLIFIC_PID,ChoiceType,Phase,Condition,Stim1,Stim2,Validity,FreeChoiceResp,Response.keys,FreeChoiceAccuracy,RT_io,SSCRAwareness,S1TRAwareness,S1CRAwareness,S2Awareness)

#table(Phase31TCraw_Awareness$FreeChoiceAccuracy,Phase31TCraw_Awareness$Validity)
##compute new variables - contrast coding
Phase31TCraw_Awareness <- Phase31TCraw_Awareness %>% 
  mutate(SSCRAwareness = ifelse(SSCRAwareness == "Aware",1,-1),
         S2Awareness = ifelse(S2Awareness=="Aware",1,-1),
         S1CRAwareness = ifelse(S1CRAwareness=="Aware",1,-1),
         S1TRAwareness = ifelse(S1TRAwareness=="Transferred",1,-1))

#write_csv(file = here("data","DataforAwarenessMLM.csv"),Phase31TCraw_Awareness)
...
Phase31TC_Awareness <- read_csv(here("data","DataforAwarenessMLM.csv"))
...
m1 <- glmer(data = Awaremlm,
            FreeChoiceAccuracy ~ 1+SSAwareness + S2RAwareness + (1|PROLIFIC_PID),
            family = binomial,
            glmerControl(optCtrl = list(maxfun = 2e5)))
...
m3 <- glmer(data = Awaremlm,
             FreeChoiceAccuracy ~ 1+S1TransferAwareness+ (1|PROLIFIC_PID),
             family = binomial)

saveRDS(m3, file = here("data","AwarenesswithS1T.rds"))

m3 <- read_csv(here("data","AwarenesswithS1T.rds"))
...
m4 <- glmer(data = Awaremlm,
            FreeChoiceAccuracy ~ 1+(SSAwareness * S1TransferAwareness) + (1|PROLIFIC_PID),
            family = binomial)
saveRDS(m4, file = here("data","AwarenesswithS1T&SS.rds"))

m4 <- read_csv(here("data","AwarenesswithS1T&SS.rds"))
...
```{r}
m4 <- glmer(data = Awaremlm,
            FreeChoiceAccuracy ~ 1+(SSAwareness * S1TransferAwareness * S2RAwareness) + (1|PROLIFIC_PID),
            family = binomial,
            glmerControl(optimizer = "bobyqa",optCtrl = list(maxfun = 2e5)))

saveRDS(m4,file = here("data","Awarenesswithall3factors.rds"))

tab_model(m4, show.stat = T, show.se = T, show.est = T, show.ci = F)
```
